...
本文描述了笔者基于 Serverless 技术开发 Chat 机器人提醒应用的全流程，记录了开发遇到的一些问题和技术选择的思考。并且开发完成后，有小伙伴想知道一些技术细节来做二次开发，于是有了这篇分享。
...
首先我们尝试使用 Python 或者任意语言在本地环境进行 Schedule 定时，直接使用本地环境进行部署运行，因为不要求高可用，偶尔的不可用也可以接受，所以在不愿使用其它技术栈的情况下也未尝不可。

方案是使用 Python schedule 库或者任意语言定时发送消息，使用本地文件来存储轮换名单。优点是在本地部署调试和管理起来比较方便，不需要额外的花费，缺点是必须保证  Python 定时进程存活和有网络，一旦电脑关机或者你请假时就不可用了🤔。如下代码 Demo 是获取当前 Owner 名字和下一个 Owner 的名字一起发送 Code Review 提醒给企业微信。
...
该方案使用云服务器来运行定时进程，和第一个方案使用的技术栈相同，可以用任意语言定时发送消息，只需要注意时区问题即可。区别在于该方案需要购买云服务器来部署，不用在意关机或者不可用问题，但是在目前这种技术需求和资源需求下，去买一个云服务器完成该功能肯定是不划算的。

以上两种方案都有各自的优缺点，一种是可用性不高，一种是费用高，所以接下来最后一种 Serverless 方案，即是可以解决这两种问题的方案，也是我们今天主要需要讲的方案。

## Serverless

无服务器计算（serverless computing），是一种由云服务商（AWS，Azure 或 Cloudflare）负责通过动态分配资源来执行一段代码的执行模型，并且仅收取运行代码所使用资源的费用。该代码通常运行在无状态的容器中，能够被包括HTTP请求、调度事件（cron任务）等各种事件触发并运行，但是不能存储数据。

因为之前分析技术需求时，我们了解到该应用的 `Request / Day < 10`，所需的 CPU 时间很少，所以使用 serverless 来执行定时发送消息的任务非常的合适，主要原因一是基础设施由云服务商提供，可用性方面有一定的保证，二是价格非常便宜(这也是 serverless 崛起的最重要的原因)。下面是各大云服务商提供的每月免费额度，我们这个项目每个月用不到免费额度的万分之一。

- AWS Lamda: 128M + 3,200,000S
- Azure Function: 1 million requests
- Cloudflare Workers: 100,000 requests

由于 serverless 是不提供存储功能的，所以需要存储人员轮换名单的话，光靠 serverless 就不够用了。当然各大云服务商也提出了不同类型的存储服务。有关系型数据库、NoSQL、KV，甚至是基于 SQLite 的 `serverless database`，例如 [Cloudflare D1](https://blog.cloudflare.com/introducing-d1/)。不过在这个应用中，我们无需使用结构化的数据，只需要很简单的 KV 存储即可。如下面几个服务和其对应的免费额度：
...
由于 Cloudflare 原生提供绑定域名和提供 `serverless database`，所以笔者比较喜欢使用 Cloudflare Workers (不过可用性方面个人认为不如 AWS)。这里就以 Cloudflare Workers 为例展示代码，代码主要分为两个 `serverless application`，也就是两个定时任务，一是定时发送消息给 Chat channel，另一个是定时轮换人员名单。

首先是定时发送消息给 Chat channel，获取当前的会议 Owner，会议地址和消息，通过配置的发送渠道（企业微信或者 Google Chat），发送给对应的 Chat channel。
...
二是定时轮换人员名单，这里主要是因为人员轮换的时间和发送消息的时间可能不同步，例如每周轮换一次 Owner，明天发送消息，所以需要单独的一个定时。
